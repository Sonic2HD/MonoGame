...
namespace Microsoft.Xna.Framework.Graphics.PackedVector
{
	...
	public struct Rgba64 : IPackedVector<ulong>, IEquatable<Rgba64>, IPackedVector
	{
		...
		{
			return (ulong) (
				(((ulong)(float)Math.Round(MathHelper.Clamp(x * 0xFFFF, 0, 65535f)) ) ) |
				(((ulong)(float)Math.Round(MathHelper.Clamp(y * 0xFFFF, 0, 65535f)) ) << 16) |
                (((ulong)(float)Math.Round(MathHelper.Clamp(z * 0xFFFF, 0, 65535f)) ) << 32) |
				(((ulong)(float)Math.Round(MathHelper.Clamp(w * 0xFFFF, 0, 65535f)) ) << 48)
            );
		}
	}
}
