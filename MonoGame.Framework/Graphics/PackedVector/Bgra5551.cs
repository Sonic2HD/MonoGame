...
namespace Microsoft.Xna.Framework.Graphics.PackedVector
{
    ...
    public struct Bgra5551 : IPackedVector<UInt16>, IEquatable<Bgra5551>, IPackedVector
    {
        ...
        {
            return (UInt16) (
                (((int) (float)Math.Round(MathHelper.Clamp(x, 0, 1) * 31.0f) & 0x1F) << 10) |
                (((int) (float)Math.Round(MathHelper.Clamp(y, 0, 1) * 31.0f) & 0x1F) << 5) |
                (((int) (float)Math.Round(MathHelper.Clamp(z, 0, 1) * 31.0f) & 0x1F) << 0) |
                ((((int) (float)Math.Round(MathHelper.Clamp(w, 0, 1)) & 0x1) << 15))
            );
        }
    }
}
