...
namespace Microsoft.Xna.Framework.Graphics.PackedVector
{
    ...
    public struct Rgba1010102 : IPackedVector<uint>, IEquatable<Rgba1010102>, IPackedVector
    {
        ...
        {
            return (uint) (
                (((int) (float)Math.Round(MathHelper.Clamp(x, 0, 1) * 1023.0f) & 0x03FF) << 0) |
                (((int) (float)Math.Round(MathHelper.Clamp(y, 0, 1) * 1023.0f) & 0x03FF) << 10) |
                (((int) (float)Math.Round(MathHelper.Clamp(z, 0, 1) * 1023.0f) & 0x03FF) << 20) |
                (((int) (float)Math.Round(MathHelper.Clamp(w, 0, 1) * 3.0f) & 0x03) << 30)
            );
        }
    }
}
